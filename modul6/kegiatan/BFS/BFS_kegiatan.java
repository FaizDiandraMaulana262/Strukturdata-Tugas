...
public class BFS_kegiatan<T> {
    ...
    public void test(int len, int ret) {
        int i = 0;
        for (Integer integer : this.vertex) {
            System.out.println(integer);
            ArrayList<?> retrievedList1 = this.vertexNeighbors.get(i);
            for (Object object : retrievedList1) {
                if (this.travelar.indexOf((Integer) object) == -1) {
                    this.travelar.add((Integer) object);
                }
            }
            i += 1;
        }
